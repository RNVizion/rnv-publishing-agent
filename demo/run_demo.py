...
FAST = {"timeout": 20, "interval": 1, "og_timeout": 6}
...
def main() -> int:
    ...
    print(f"{DIM}  The share image is generated by a separate CI job, so it is not on disk yet.\n"
          f"  The chain should publish anyway and say so loudly.{RESET}")
    ...
    if warned:
        print(f"  {YELLOW}warning surfaced:{RESET} {r3['warnings'][0]}")

    # ---- Act 3b --------------------------------------------------------------
    banner("3b", "The image catches up", "the same run, when CI finishes in time")
    ...
    threading.Thread(target=build_og_late, daemon=True).start()
    print(f"{DIM}  A background job writes the share image two seconds from now,\n"
          f"  the way the build-og Action lands after the push.{RESET}")
    r3b = server.publish_post("late-arrival", for_real=True, **FAST)
    live_step = next(s for s in r3b["trace"] if s["step"] == "wait_for_live")
    ok3b = r3b["ok"] is True and live_step.get("og_image_live") is True and not r3b.get("warnings")
    print(f"  {GREEN if ok3b else RED}{'PASS' if ok3b else 'FAIL'}{RESET} "
          f"polled until the image appeared; published with no warning")
