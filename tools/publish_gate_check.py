"""
Guard: the for_real gate holds, and a dry run writes nothing. MANUAL, against production.

Moved out of tests/ on purpose. Named test_publish_gate.py it looked like a pytest
module and collected zero tests, so CI went green whether or not this guard ever ran:
the same failure this file's own header describes in client_test.py, wearing different
clothes. The pytest form now lives at tests/test_mcp_surface.py and runs on every push
against a throwaway fixture; this copy stays for the thing that one cannot do, which is
run against the real site checkout.
...
Invoke by whatever path reaches it:
    ...
    python tools/publish_gate_check.py                # from the repo root
    python /workspaces/rnv-publishing-agent/tools/publish_gate_check.py
